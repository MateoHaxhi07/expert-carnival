...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Seller_Category" in df.columns:
        # Ensure it's a string column where possible, but keep NaN
...
def choose_seller_categories_fixed(df: pd.DataFrame):
    ...
    Returns (selection_keys, label)
      - selection_keys is a list among {'ALL','BAR','DELIVERY','RESTAURANT','NAN'}
    """
...
def filter_by_seller_category_fixed(df: pd.DataFrame, keys) -> pd.DataFrame:
    ...
    Apply fixed-key filtering:
      - BAR         -> Seller_Category == 'Bar' (case-insensitive)
      - DELIVERY    -> 'Delivery'
      - RESTAURANT  -> 'Restaurant'
      - NAN         -> isna() OR '' OR literal 'NaN' (any case/extra spaces)
      - ALL         -> no filter
    """
    ...
    if "BAR" in keys:
        mask_total |= (s_norm == "bar")
    if "DELIVERY" in keys:
        mask_total |= (s_norm == "delivery")
    if "RESTAURANT" in keys:
        mask_total |= (s_norm == "estaurant")
    if "NAN" in keys:
        # True NaN OR empty strings OR literal 'nan'
...
# -------------------------------------------------------------------------
# 2b) Separate seller data (same as before)
# -------------------------------------------------------------------------
def separate_seller_data(df: pd.DataFrame):
    """
    Split data into:
    1. Main report data (excluding specified sellers)
...
class ReportMetrics:
    ...
    def calculate_menu(self):
        ...
        total_rev = float(menu["Revenue"].sum())
    ...
    def calculate_time(self):
        ...
        total_rev = float(periods["Revenue"].sum())
...
# -------------------------------------------------------------------------
# 4b) Seller-specific analysis (unchanged)
# -------------------------------------------------------------------------
class SellerAnalysis:
    ...
    def calculate_all(self):
        ...
        if SELLER_SHEET_SHOW_ITEM_BREAKDOWN:
            ...
            total_rev = float(items["Revenue"].sum())
...
# 6) Excel / TXT output
# -------------------------------------------------------------------------
def generate_excel_report(metrics: dict, seller_analyses: dict, period_name: str, output_filename: str, df_current_filtered: pd.DataFrame):
    ...
    with pd.ExcelWriter(output_filename, engine="xlsxwriter") as writer:
        ...
        exec_df = pd.DataFrame({
            "Metric": [
                "Total Revenue", "Total Orders", "Average Order Value", "Orders per Day",
                "Revenue Change vs Previous", "Orders Change vs Previous"
            ],
            "Value": [
                ex["total_revenue"], ex["total_orders"], ex["avg_order_value"],
                round(ex["orders_per_day"], 1),
                f'{ex.get("revenue_change", 0):.1f}%',
                f'{ex.get("orders_change", 0):.1f}%'
            ]
        })
        ...
        if not daily_df.empty:
            ...
            fmt_headers(ws)
            fmt_currency(ws, [2, 4])  # Revenue, AvgOrderValue
        ...
        if not time_df.empty:
            ...
            fmt_headers(ws)
            fmt_currency(ws, [1, 3])  # Revenue, AOV
...
def main():
    ...
    calc = ReportMetrics(df_cur, df_prev)
    ...
    # write
    generate_excel_report(metrics, seller_analyses, f"{period_name} — {cats_label}", out_name, df_cur)
